...
@Component
public class CalculateKpiHighNumberBestIndicatorImpl implements CalculateKpiHighNumberBestIndicator {

    /*
    Regla: Valores numéricos siendo 0 el valor de representación de peor salud
    y a mayor número, mejor resultado, tendiendo a infinito.

    KPIs posibles: Ficticios, memoria libre, ganancias, recursos disponibles
    ...
    public CalculateKpiHighNumberBestIndicatorImpl() {

    }

    public CalculateKpiHighNumberBestIndicatorImpl(
            double warningKpiThreshold,
            double criticalKpiThreshold,
            double warningIdxThreshold,
            double criticalIdxThreshold,
            double value) {
    ...
    public double calculateIndex() {
        ...
        } //((atan(n-a)/escale)/atan(9999999..))*(y-x)+x
        //Si no hay infinitos
        else {
    ...
    private Double determineA() {
        if (value <= criticalKpiThreshold) {
            return 0d;
        } else if (value <= warningKpiThreshold) {
